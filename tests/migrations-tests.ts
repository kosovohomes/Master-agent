...
try {
  ...
  if (canCreate) {
    try {
      ...
      check("empty-db: flags seed present (13 incl. ai_gateway + knowledge_v2 + connectors + research + content + seo + social + marketing + sales + analytics)", seededFlags.rows[0].n === 13);
      await pool.end();
