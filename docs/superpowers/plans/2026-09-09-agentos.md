...
**Files:**
- Create: `lib/channels.ts` (Task 6 was dispatched with this name; supersedes the earlier `lib/encrypt.ts` naming)
- Test: `tests/channels-tests.ts`
...
```ts
export function encryptChannelToken(plaintext: string, key?: Buffer): string; // iv:tag:cipher (hex)
export function decryptChannelToken(payload: string, key?: Buffer): string;
```

- [ ] **Step 1: Write failing test**

Create `tests/channels-tests.ts`:
...
check("roundtrip works", decryptChannelToken(c1, key) === "secret-token-123");
check("wrong key fails", (() => { try { decryptChannelToken(c1, Buffer.alloc(32, 1)); return false; } catch { return true; } })());
...
- [ ] **Step 2: Implement `lib/channels.ts`**
...
Tests additionally assert: tampered ciphertext throws (GCM auth), malformed payload throws, missing `CHANNEL_ENC_KEY` throws a clear error at call time, output is hex `iv:tag:cipher`.

- [ ] **Step 3: Run tests**

Run: `npm test`
Expected: CHANNELS SUITE PASS.

- [ ] **Step 4: Commit**

```bash
git add lib/channels.ts tests/channels-tests.ts
...
**Interfaces:**
- Consumes: `lib/agents/approval.ts` (`markPosted`, `markFailed`, `listDraftsByTenant`), `lib/db.ts`, `lib/channels.ts` (`decryptChannelToken` for reading stored tokens), `.env.local` (Resend key, channel tokens stored per tenant in `channels.token_encrypted`).
- Produces:
...
Note: the sweep calls `ctx.publish` so the test can stub it; the production route wires `ctx.publish` to a `getPublisher(kind)` call plus `decryptChannelToken` on the token.
...
Replace the `files` array with: `smoke-tests.ts`, `db-tests.ts`, `llm-tests.ts`, `channels-tests.ts`, `agents-routing-tests.ts`, `approval-tests.ts`, `rag-tests.ts`, `generators-tests.ts`, `dispatch-tests.ts`, `chat-tests.ts`, `publishers-tests.ts`, `widget-tests.ts`, `admin-tests.ts`, `security-tests.ts`.
...
// tokens must never be stored in plaintext: sweep route relies on decryptChannelToken
